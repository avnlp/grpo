# This implementation is based on: https://github.com/McGill-NLP/nano-aha-moment/blob/main/utils.py
...
def prepare_model_inputs(
    ...
    """
    max_seq_len = max(len(q) + len(r) for q, r in zip(query_token_ids, response_token_ids, strict=False))
    inputs = {"input_ids": [], "attention_mask": [], "labels": [], "advantages": [], "labels_mask": []}

    pad_token_id = 0  # Doesn't matter, will be masked
    ignore_index = -100

    for query, response, advantage in zip(query_token_ids, response_token_ids, advantages, strict=False):
...
def find_last_checkpoint(exp_dir: Path) -> tuple[Path | None, int | None]:
...
def load_model_into_vllm(model: DeepSpeedEngine | PreTrainedModel, llm: LLM) -> None:
...
def clean_up_checkpoints(
    exp_dir: Path, keep_every_n_steps: int | None = None, exclude: list[Path] | None = None
) -> None:
