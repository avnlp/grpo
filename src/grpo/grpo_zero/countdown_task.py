...
def answer_reward_function(response: str, numbers: list[int] | None = None, target: int | None = None) -> float:
    ...
    try:
        result = literal_eval(answer_content)
        if abs(float(result) - float(target)) < 1e-5:
            return 1.0
    except Exception as e:
        msg = f"Answer does not evaluate to the target. {e}"
